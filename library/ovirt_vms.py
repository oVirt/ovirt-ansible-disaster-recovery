...
options:
    ...
    vnic_profile_mappings:
        description:
            ...
            " This attribute will be used usually when there are multiple vnic profiles in different data centers."
            " The dc name will distinguish only the relevant vnic profiles the VM should be registered to."
            " If the dc name does not exists in the setup, then no vnic profiles will be used in the register operation."
            " If the dc name will be used, all the vnic profiles will be passed."
...
def _get_vnic_profile_mappings(module, dc_name):
    ...
    for vnicProfileMapping in module.params['vnic_profile_mappings']:
        ...
        if target_dc is None or target_dc == 'EMPTY_ELEMENT' or target_dc == dc_name:
    ...
    try:
        ...
        if state in ('present', 'running', 'next_run'):
            ...
            if vm is None:
                ...
                filtered_vnic_profiles = _get_vnic_profile_mappings(module, dc_name)
                vm_service.register(
                    ...
                    vnic_profile_mappings=filtered_vnic_profiles
                    if module.params['vnic_profile_mappings'] and filtered_vnic_profiles else None,
